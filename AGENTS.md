...
1. `can/<name>adapter.h/.cpp` — 继承 `CanInterface`，实现 6 个纯虚函数；`scanDevices()` 填 `CanDeviceInfo` 时按复合编码写 `channel`。
...
- **CAN-FD 数据域波特率无人消费**：`dataBaudText` 目前只用于同步 UI 文本，ZCANFD 在 `open()` 中硬编码数据域 2MHz，其他适配器直接忽略。
