...
void MainWindow::showWelcomePage()
{
    ...
    // 清掉上一个待销毁的实例，避免与新实例同时留在布局中
    QWidget *page0 = m_stack->widget(0);
    QLayout *layout = page0->layout();
...
void MainWindow::setupMenuBar()
{
    ...
    connect(tileHAct, &QAction::triggered, this, [this]() { tileDockAreas(Qt::Horizontal); });
    windowMenu->addAction(tileHAct);

    QAction *tileVAct = new QAction(tr("垂直平铺"), this);
    connect(tileVAct, &QAction::triggered, this, [this]() { tileDockAreas(Qt::Vertical); });
    ...
    connect(unsplitAct, &QAction::triggered, this, [this]() {
        ...
        for (int i = 1; i < areas.size(); ++i) {
            const auto docks = areas[i]->dockWidgets();
            for (auto *dw : docks)
...
/// 把当前所有会话改为水平/垂直平铺并均分尺寸
void MainWindow::tileDockAreas(Qt::Orientation orientation)
{
    const QList<ads::CDockAreaWidget*> areas = m_dockManager->openedDockAreas();

    QList<ads::CDockWidget*> allDocks;
    for (auto *a : areas)
        allDocks.append(a->dockWidgets());
    if (allDocks.size() < 2) return;

    const ads::DockWidgetArea area = (orientation == Qt::Horizontal)
        ? ads::RightDockWidgetArea : ads::BottomDockWidgetArea;

    ads::CDockAreaWidget *targetArea = areas.first();
    for (int i = 1; i < allDocks.size(); ++i)
        targetArea = m_dockManager->addDockWidget(area, allDocks[i], targetArea);
